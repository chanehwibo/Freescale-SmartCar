...
static void SD_SetBaudRate(uint32_t baudrate)
{
    ...
    for (pres = 2; pres <= 256; pres <<= 1)
    {
        ...
        {
            val = pres * div * baudrate - SystemClock*1000000;
            if (val >= 0)
            {
                if (min > val)
                {
...
uint32_t SD_QuickInit(uint32_t baudrate)
{
    ...
    SD_InitStruct1.baudrate = baudrate;
	  gpio_alf_init(PTE,0,ALF4);
    gpio_alf_init(PTE,1,ALF4);
	  gpio_alf_init(PTE,2,ALF4);
	  gpio_alf_init(PTE,3,ALF4);
	  gpio_alf_init(PTE,4,ALF4);
	  gpio_alf_init(PTE,5,ALF4);
